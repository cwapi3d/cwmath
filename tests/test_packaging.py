...
def test_wheel_excludes_tests(tmp_path: Path) -> None:
    ...
    assert any(name == 'cwmath/py.typed' or name.endswith('/cwmath/py.typed') for name in names)
    assert not any(name.startswith('cwmath/tests/') or name.startswith('tests/') for name in names)
